...
|------|------|----------|------|
| A（框架） | main | 本框架搭建 | ✅ 已完成 |
| B | feature/board | 白板实时协作（Fabric.js 画布 + Socket 事件 + 版本快照） | ✅ 已完成 |
...
- [x] 防抖自动保存（停止操作 1.2s 后落库）+ 手动保存 + 智能自动版本快照（30 秒有变更则创建）
- [x] 实时多人同步：画布全量广播（`board:sync`）、多人光标位置（`board:cursor`）
- [x] Socket 错误事件：`board:error`（权限不足时服务端回传，前端 Toast 提示）
- [x] 版本恢复：服务端广播 `board:version-restored`，前端重新拉取画布内容
- [x] 导出 PNG（`canvas.toDataURL()`）
- [x] 进入房间时从后端加载已保存的画布内容

> 注：实时同步使用 canvas JSON 整体广播方案，未使用 Y.js CRDT，多人同时操作时后写覆盖先写。
...
- [x] Socket 错误事件：`doc:error`（权限不足时服务端回传，前端 Toast 提示）

### 用户认证 & 项目管理已完成的能力清单（D）

- [x] 登录页（`LoginPage`）：邮箱 + 密码表单，登录态写入 Zustand + localStorage；开发模式下支持调试预览模式（绕过真实登录）
- [x] 注册页（`RegisterPage`）：用户名 / 邮箱 / 密码 + 二次确认，注册成功后跳转登录
- [x] 首页（`HomePage`）：展示当前用户可访问的全部白板/文档项目（按更新时间排序）
- [x] 新建项目：支持选择类型（Whiteboard / Document），创建后直接跳转对应协作页
- [x] 成员权限管理弹窗：owner 可增删成员（通过邮箱）、调整 editor/viewer 角色
- [x] 项目重命名、删除（owner 专属）
- [x] 项目统计卡片：项目总数 / 白板数 / 文档数
- [x] 后端 `/api/items/*` 全套接口：`GET/POST /api/items`、`GET /api/items/:id`、`PUT /api/items/:id/permissions`、`PUT /api/items/:id/title`、`DELETE /api/items/:id`

### 版本快照 & 评论模块说明（E）

版本快照与评论功能已在开发过程中并入文档模块（C）实现，不再单独维护。
具体能力见「文档模块已完成的能力清单」中的版本、评论相关条目。
