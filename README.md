...
| `handlers.js` | 为每条连接注册具体事件：`join`/`leave`（调通用房间方法，`join` 会按 item 类型做白板/文档权限校验）、`ping`/`pong`（联调自测）。**C 文档协作**：`doc:operation`、`doc:cursor`、`doc:title-changed`、`doc:sidebar-changed`（权限不足时回传 `doc:error`）；**B 白板协作（Yjs）**：`board:yjs-update`、`board:yjs-cursor`、`board:yjs-sync-request`、`board:yjs-sync-state`（viewer 补发失败静默）；**文档树**：`tree:subscribe`/`tree:unsubscribe`。Socket 权限校验带 15s TTL 缓存；数据库抖动时对已授权用户沿用上次授权，避免实时绘制被误拒 |
...
- **响应拦截**：
  ...
  - `code === 40101`（token 失效）→ 先用 `refreshToken` 静默换新 token 并重试原请求；若 refresh 也失败，再清理登录态并跳转 `/login`
...
- **状态枚举**：暴露 `status` 字段（`disconnected / connecting / connected / reconnecting / recovered`），各页面自行读取显示连接状态（DocPage 顶部离线横幅、BoardPage 标题区连接标签）
...
| 路径 | 组件 | 是否受保护 | 状态 |
|------|------|-----------|--------|
| `/login` | LoginPage | 否 | ✅ 已实现 |
| `/register` | RegisterPage | 否 | ✅ 已实现 |
| `/` | HomePage | 是 | ✅ 已实现 |
| `/board/:id` | BoardPage | 是 | ✅ 已实现 |
| `/doc/:id` | DocPage | 是 | ✅ 已实现 |
